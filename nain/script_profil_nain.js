const nameNain = document.getElementById("name");
...
const liRich = document.getElementById("rich");
...
// Afficher le profil
nameNain.innerText = `${currentUser.name}`
liLine.innerText = `Mon sang : ${currentUser.bio.line}`;
liKing.innerText = `J'ai taillé ma vie en ses murs : ${currentUser.bio.king}`;
liDonation.innerText = `Dans nos mines, je suis : ${currentUser.bio.act}`;
liRich.innerText = `Ce que mon coeur convoite : ${currentUser.bio.rich}`;
        ...
        if (!userSearch || currentUser.name === userSearch.name || currentUser.following?.includes(userSearch.id)) {
            ...
            <div class="profil" id="search-result-profile">
                ...
                <div class="btn-accept">
                    <button id="btn-accept-friend" class="btn-accept-profil">J'y consens, par ma barbe !</button>
                    <button id="btn-refuse-friend" class="btn-accept-profil">Je refuse - que mon marteau ne résonne pas pour ceci</button>
                </div>
            </div>
            `;
