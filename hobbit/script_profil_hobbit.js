const nameHobbit = document.getElementById("name");
...
const liPassion = document.getElementById("passion");
...
nameHobbit.innerText = `${currentUser.name}`
liLine.innerText = `Mon sang : ${currentUser.bio.line}`;
liKing.innerText = `Ma Terre : ${currentUser.bio.king}`;
liDonation.innerText = `Le rôle qui m'a été confié : ${currentUser.bio.act}`;
liPassion.innerText = `Ce qui fait danser mon âme : ${currentUser.bio.loisir}`;
        ...
        if (!userSearch || currentUser.name === userSearch.name || currentUser.following?.includes(userSearch.id)) {
            ...
            <div class="profil" id="search-result-profile">
                ...
                <div class="btn-accept">
                    <button id="btn-accept-friend" class="btn-accept-profil">Ça me ferait plaisir de te compter parmi mes amis.</button>
                    <button id="btn-refuse-friend" class="btn-accept-profil">Non merci, mais je te souhaite bonne route.</button>
                </div>
            </div>
            `;
