const nameElfe = document.getElementById("name");
...
nameElfe.innerText = `${currentUser.name}`
liLine.innerText = `Mon sang : ${currentUser.bio.line}`;
liKing.innerText = `Mon allégence : ${currentUser.bio.king}`;
liDonation.innerText = `L'ensaignement qui m'a été confié : ${currentUser.bio.act}`;
liPassion.innerText = `Le don qui m'a été offert : ${currentUser.bio.passion}`;
        ...
        if (!userSearch || currentUser.name === userSearch.name || currentUser.following?.includes(userSearch.id)) {
            ...
            <div class="profil" id="search-result-profile">
                ...
                <div class="btn-accept">
                    <button id="btn-accept-friend" class="btn-accept-profil">Ainsi soit-il, sous les étoiles immortelles.</button>
                    <button id="btn-refuse-friend" class="btn-accept-profil">Que ce nom suive un autre sentier que le mien.</button>
                </div>
            </div>
            `;
